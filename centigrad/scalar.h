...
namespace autodiff
{
	...
	public:
		...
		friend Scalar operator / (double obj, Scalar& other) {
			Scalar v(obj);
			return v / other;
		}
		...
		Scalar operator ^ (Scalar& other) {
			Scalar* p = &other;
			vector<Scalar*> set = { this,p };
			Scalar out((this->value * other.value), set, '^');

			out._backwards = [this, &other, out]()
				{
					this->grad += (other.value * std::pow(this->value, other.value - 1)) * out.grad;
				};
			return out;
		}
		Scalar operator ^ (double num) { //done
			Scalar n(num);
			return *this ^ n;
		}
		friend Scalar operator ^ (double num, Scalar& other) {
			Scalar res(num);
			return res ^ other;
		}
